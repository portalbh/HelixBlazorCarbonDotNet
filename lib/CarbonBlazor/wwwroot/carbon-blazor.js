...
export function initAppShell() {
  ...
  const themeSelect = document.querySelector('[data-cb-theme-select]');
  ...
  if (stored && themeRoot && themeMap[stored]) {
  ...
  if (themeSelect && !themeSelect.dataset.cbShellBound) {
    themeSelect.dataset.cbShellBound = 'true';
    themeSelect.addEventListener('change', (event) => {
      const value = event.target.value;
      if (themeRoot && themeMap[value]) {
        themeRoot.dataset.theme = themeMap[value];
      }
